#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Unified response module with selectable NumPy / CuPy backend.

Functions mirror ``response.py`` (CPU) and ``response_gpu.py`` (GPU) while
using a single flag ``use_gpu`` to switch backends. When ``use_gpu`` is True
and CuPy is available, all array math is routed to the GPU; otherwise NumPy is
used. Numerical behaviour matches the original modules.
"""
...
try:  # Optional CuPy dependency
    import cupy as cp
    _has_cupy = True
except Exception:  # pragma: no cover - CuPy not installed or unavailable
    cp = None
    _has_cupy = False
...
def _matrix_res_pro_cpu(n, p):
    """TensorProduct(n, n) : P, where A:B = A_ij B_ij (NumPy/Numba)."""
...
def _to_xp(arr, xp):
    """Convert input to the chosen backend (avoids unnecessary copies)."""
    if xp is np:
...
def _sinc(x, xp):
    """Backend-agnostic sinc: sin(pi x)/(pi x)."""
    pi_x = xp.pi * x
    return xp.where(x == 0, xp.ones_like(x), xp.sin(pi_x) / pi_x)


def get_y_slr_td(wf, tf, det="TQ", TDIgen=1, use_gpu=False):
    """Time-domain y_slr with selectable backend."""
    if TDIgen == 1:
        TDI_delay = 4
    elif TDIgen == 2:
        TDI_delay = 8
    ...
    tf_cpu = np.asarray(tf)  # detector classes expect NumPy input
    ...
    def h_tdi_delay(tf_s, xi_p, xi_c):
        # wf.get_hphc expects NumPy arrays
        def to_cpu(arr):
...
def tdi_XYZ2AET(X, Y, Z, use_gpu=False):
    """Convert XYZ to AET for the selected backend."""
    xp = _get_xp(use_gpu)
    A = 1 / xp.sqrt(2) * (Z - X)
...
def get_XYZ_td(wf, tf, det="TQ", TDIgen=1, use_gpu=False):
    """Generate time-domain TDI XYZ for different generations."""
    xp = _get_xp(use_gpu)
...
def get_AET_td(wf, tf, det="TQ", TDIgen=1, use_gpu=False):
    """Generate time-domain TDI AET for different generations."""
    X, Y, Z = get_XYZ_td(wf, tf, det, TDIgen, use_gpu)
    return tdi_XYZ2AET(X, Y, Z, use_gpu)


def trans_y_slr_fd(vec_k, p, det, f, use_gpu=False):
    """Frequency-domain y_slr transfer functions (needs det object)."""
    xp = _get_xp(use_gpu)
...
def trans_XYZ_fd(vec_k, p, det, f, TDIgen=1, use_gpu=False):
    """Calculate XYZ transfer functions in frequency domain."""
    xp = _get_xp(use_gpu)
...
def trans_AET_fd(vec_k, p, det, f, TDIgen=1, use_gpu=False):
    """Calculate AET transfer functions in frequency domain."""
    xp = _get_xp(use_gpu)
    y_slr_list = trans_y_slr_fd(vec_k, p, det, f, use_gpu)
    Dt = xp.exp(2j * xp.pi * f * det.L_T)  # Time delay factor
    Dt2 = Dt * Dt
...
# Backward-compatible aliases matching previous GPU helpers
