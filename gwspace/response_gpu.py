...
# -*- coding: utf-8 -*-
"""
GPU-enabled version of gwspace.response.

Functions mirror gwspace.response but accept a `use_gpu` flag (default: True).
When `use_gpu` is True and CuPy is available, array math is performed on GPU;
otherwise it falls back to NumPy seamlessly.
"""

import numpy as np
from gwspace.Orbit import detectors

try:  # CuPy is optional; we fall back to NumPy if not present
    import cupy as cp
    _has_cupy = True
...
def _to_xp(arr, xp):
    """Convert input to the chosen backend (NumPy/CuPy) without unnecessary copies."""
    if xp is np:
...
def _sinc(x, xp):
    """Backend-agnostic sinc implementation: sin(pi x)/(pi x)."""
    pi_x = xp.pi * x
...
def get_XYZ_td(wf, tf, det="TQ", TDIgen=1, use_gpu=True):
    """Generate TDI XYZ in different TDI generation (GPU-aware)."""
    xp = _get_xp(use_gpu)
...
def get_AET_td(wf, tf, det="TQ", TDIgen=1, use_gpu=True):
    """Generate TDI AET in different TDI generation (GPU-aware)."""
    X, Y, Z = get_XYZ_td(wf, tf, det, TDIgen, use_gpu)
    return tdi_XYZ2AET(X, Y, Z, use_gpu)


def trans_y_slr_fd(vec_k, p, det, f, use_gpu=True):
    """Frequency-domain y_slr transfer functions (GPU-aware).

    Unlike the time-domain version, this returns transfer functions which should
    be multiplied by the original waveform in frequency domain manually.
    """
    xp = _get_xp(use_gpu)
...
def trans_XYZ_fd(vec_k, p, det, f, TDIgen=1, use_gpu=True):
    """Calculate XYZ transfer functions in frequency domain (GPU-aware)."""
    xp = _get_xp(use_gpu)
...
def trans_AET_fd(vec_k, p, det, f, TDIgen=1, use_gpu=True):
    """Calculate AET transfer functions in frequency domain (GPU-aware)."""
    xp = _get_xp(use_gpu)
    y_slr_list = trans_y_slr_fd(vec_k, p, det, f, use_gpu)
