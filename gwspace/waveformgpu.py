...
try:
    ...
    try:
        ...
        pyIMRD = None  # optional; only BHB IMRPhenomD features need it
        pyIMRc = None
        use_py_phd = False
...
try:
    ...
    libFastGB = None  # optional; only GB waveform/response need it
...
class BHBWaveform(BasicWaveform):
    ...
    def get_amp_phase(self, delta_f=None, f_min=None, f_max=1., f_ref=0., f_series=None):
        ...
        """
        if pyIMRD is None:
            raise ImportError("PyIMRPhenomD extension is required for BHB IMRPhenomD waveforms but is not available")
        if f_series is None:
...
class GCBWaveform(BasicWaveform):
    ...
    def get_fastgb_fd_single(self, dt, oversample=1, detector='TianQin', buffer=None):
        ...
        if libFastGB is None:
            raise ImportError("libFastGB extension is required for GB waveforms but is not available")
...
class EMRIWaveform(BasicWaveform):
    ...
    def __init__(self, M, mu, a, p0, e0, x0, dist, qS, phiS, qK, phiK, T_obs,
                 Phi_phi0=0, Phi_theta0=0, Phi_r0=0, backend=None, force_backend=None, **kwargs):
        self.M = M
        self.mu = mu
        self.a = a
        self.p0 = p0
        ...
        self.x0 = x0
        self.dist = dist
        # allow users to explicitly choose FEW backend: "cpu", "cuda11x", "cuda12x", or "gpu"/"cuda"
        self._backend_request = backend if backend is not None else force_backend
    ...
    def _select_backend(self):
        ...
        def normalize(name):
            if name in ("gpu", "cuda"):
                return "cuda12x" if few.has_backend("cuda12x") else "cuda11x"
            return name

        requested = normalize(self._backend_request)
        candidates = []
        if requested:
            candidates.append(requested)
        candidates.extend(["cuda12x", "cuda11x", "cpu"])

        for name in candidates:
            try:
                if few.has_backend(name):
                    few.get_backend(name)
                    return name
            except Exception:
                continue
        return None

    def _gen_wave_func(self):
        ...
        self._backend = backend  # cache for harmonic mode path
        ...
        # FEW 2.0 的 stock 类支持 force_backend
        return GenerateEMRIWaveform(
            ...
            # 如需更细地定制模式选择，也可以传 mode_selector_kwargs={}
        )


    def get_harmonic_mode(self, eps=1e-5, model_insp="SchwarzEccFlux"):
                          #, use_gpu=False):
        ...
        # 已在 _gen_wave_func() 里选过 backend；沿用该设置
        backend = getattr(self, "_backend", None)
        if backend is None:
            backend = self._select_backend()

        # 生成稀疏轨道
        traj = EMRIInspiral(func=model_insp, force_backend=backend)
        t, p, e, x, Phi_phi, Phi_theta, Phi_r = traj(self.M, self.mu, self.a, self.p0, self.e0, 1.0)

        # 幅度与 Ylm 发生器（注意 2.0 用 force_backend；GetYlms 的 use_gpu 参数已废弃）
        amp = RomanAmplitude(force_backend=backend)
        ylm_gen = GetYlms(include_minus_m=True, force_backend=backend)

        # 2.0 的 ModeSelector 不再接收 mask / use_gpu，而是接收“发生器对象”
        mode_selector = ModeSelector(
            amplitude_generator=amp,
            ylm_generator=ylm_gen,
            force_backend=backend,
        )

        # 2.0 的 __call__ 接口：直接传 (t, a, p, e, xI, theta, phi) 等上下文
        teuk_modes_in, ylms_in, ls, ms, ns = mode_selector(
            t,
            self.a,
            p,
            e,
            x,                     # xI（inclination cosine）
            self.theta, self.phi,  # 观测方向
            mode_selection_threshold=eps,   # 以前的 eps 用这个
